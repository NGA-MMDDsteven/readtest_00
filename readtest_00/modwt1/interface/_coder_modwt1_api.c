...
static real_T c_emlrt_marshallIn(const emlrtStack *sp, const mxArray *lev, const
  char_T *identifier);
  ...
  char_T *identifier)
{
  ...
  y = d_emlrt_marshallIn(sp, emlrtAlias(lev), &thisId);
  emlrtDestroyArray(&lev);
  return y;
}
 ...
 * Return Type  : void
 */
void modwt1_api(const mxArray * const prhs[2], int32_T nlhs, const mxArray *
                plhs[1])
{
  emxArray_real_T *x;
  emxArray_real_T *w;
  ...
  lev = c_emlrt_marshallIn(&st, emlrtAliasP(prhs[1]), "lev");
...
/*
 * File trailer for _coder_modwt1_api.c
 *
 * [EOF]
 */
